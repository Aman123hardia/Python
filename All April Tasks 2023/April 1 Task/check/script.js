...
function spin(d){
   ...
   debugger
    ...
    pieslice = rotation/data.length,
    rng      = Math.floor((Math.random() * rotation) + 360);
...
const range = document.getElementById('vol-control');
const colorBox = document.getElementById('vol');

range.addEventListener('input', () => {
  const value = range.value;
  const color = `rgb(${value}, ${255 - value}, ${value})`;
